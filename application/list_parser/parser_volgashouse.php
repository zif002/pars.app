...
function getBigImage($url,$i=1){
	...
	if( count($data->find('#catalog-detail-main-image a')) ){
		$img =  $data->find('#catalog-detail-main-image a',0);

			 	
		// }
		//echo $img;
		if( !preg_match('#^http://#',$img->href) )$img->href = 'http://volgoshoes.ru'.$img->href;	
	  	//echo $img->href;
	  	$ch = curl_init();  
	...
	if($flag){
		...
		//Вывод названия товара
		 if( count($data->find('h1#pagetitle')) ){
		 	$product_title = $data->find('h1#pagetitle',0);	
		...
		//Вывод цены
		if(  count($data->find('.catalog-detail-price'))){	 		
		 	$price = $data->find('.catalog-detail-price',2);		 
		 ...
		 //Артикул
		 if(  count($data->find('.article'))){	 		
		  	$article = $data->find('.article',0);	  
		  	$article = $article->plaintext;
		  	echo $article."<br>";
		  }else{			
		 
		  }
		 if(count($data->find('.offer-name'))){
		
			$size = $data->find('.offer-name',0);
			$size = $size->plaintext;
			echo "$size";  	
		...
		//Вывод ,наличие шт
		 if( count($data->find('td.catalog-detail-image')) ){
		 	$brand = $data->find('td.catalog-detail-image div',2);
...
function getYandexImages($url,$findpages = true,$i=1,$n=200){
	...
	if( $findpages and count($data->find('.catalog-item-title a'))){
		foreach($data->find('.catalog-item-title a') as $a){	
			//$a->href.'<br>';
			// довольно распространенный случай - локальный URL. Поэтому иногда url надо дополнять до полного
			if( !preg_match('#^http://#',$a->href) )$a->href = 'http://volgoshoes.ru'.$a->href;
