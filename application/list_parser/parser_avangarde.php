...
function getBigImage($url,$i=1){
	...
	if( count($data->find('#zoom_01')) ){
		$img =  $data->find('#zoom_01',0);
		$img = $img->getAttribute('data-zoom-image');
		echo $img;
			 	
		// }
		//echo $img;
		if( !preg_match('#^http://#',$img) )$img = 'http://xn--80aaagghh4d3a.xn--p1ai'.$img;	
	  	//echo $img->href;
	  	$ch = curl_init();  
	...
	if($flag){
		...
		//Вывод названия товара
		 if( count($data->find('h1.category-title')) ){
		 	$product_title = $data->find('h1.category-title',0);	
		...
		//Вывод цены
		 if(count($data->find('.properties ul'))){
				 	$prop = array();
					foreach($data->find('.properties ul li') as $prop_temp) {	  	
							//$prop = $prop->plaintext;
							$prop[] = $prop_temp;
							echo $prop_temp->plaintext."<br>";
	 ...
	 //  		echo $color;
		...
		// }	
		//Вывод ,наличие шт
	// 	 if( count($data->find('.sizes-table')) ){
	// 	 	$size = $data->find('.sizes-table',0);
	// 		foreach ($size->find(".row") as $key) {
	// 			$row = $size->find(".row",0);
	// 			foreach ($key->find('span.green.residue') as $size_rezult) {
				
	// 					$size = $row->find('.column.column_th');
	// 					echo $size."<br>";
...
function getYandexImages($url,$findpages = true,$i=1,$n=200){
	...
	if( $findpages and count($data->find('a.p_in'))){
		foreach($data->find('a.p_in') as $a){

			//$a->href.'<br>';
			// довольно распространенный случай - локальный URL. Поэтому иногда url надо дополнять до полного
			if( !preg_match('#^http://#',$a->href) )$a->href = 'http://xn--80aaagghh4d3a.xn--p1ai'.$a->href;
