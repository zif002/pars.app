...
function getBigImage($url,$i=1){
	...
	if( count($data->find('.b-centered-image')) ){
		$img =  $data->find('.b-centered-image',0);

			 	
		// }
		//echo $img;
		if( !preg_match('#^http://#',$img->href) )$img->href = 'http://slivki.tiu.ru/'.$img->href;	
	  	//echo $img->href;
	  	$ch = curl_init();  
	...
	if($flag){
		...
		//Вывод цены
		if(  count($data->find('.b-product__price'))){	 		
		 	$price = $data->find('.b-product__price',0);		 
		 ...
		 if(count($data->find('.b-user-content'))){
		 	$size1 = array();
			foreach($data->find('.b-user-content p') as $size) {	  	
					$size = $size->plaintext;
					$size1[] = $size;
					echo $size."<br>";
...
function getYandexImages($url,$findpages = true,$i=1,$n=3){
	...
	if( $findpages and count($data->find('.b-product-line__product-name-link'))){
		foreach($data->find('.b-product-line__product-name-link') as $a){	
			//$a->href.'<br>';
			// довольно распространенный случай - локальный URL. Поэтому иногда url надо дополнять до полного
			if( !preg_match('#^http://#',$a->href) )$a->href = 'http://slivki.tiu.ru/'.$a->href;
