...
function getBigImage($url,$i=1){
	...
	if( count($data->find('#item-image a')) ){
		$img =  $data->find('#item-image a',0);

			 	
		// }
		//echo $img;
		if( !preg_match('#^http://#',$img->href) )$img->href = 'http://trikotyata.ru'.$img->href;	
	  	//echo $img->href;
	  	$ch = curl_init();  
	...
	if($flag){
		...
		//Вывод названия товара
		 if( count($data->find('h1.item-title')) ){
		 	$product_title = $data->find('h1.item-title',0);	
		...
		//Вывод цены
		if(count($data->find('.item-price'))){	 		
		 	$price = $data->find('.item-price',0);		 
...
function getYandexImages($url,$findpages = true,$i=1,$n=3){
	...
	if( $findpages and count($data->find('a.catimg'))){
		foreach($data->find('a.catimg') as $a){	
			//$a->href.'<br>';
			// довольно распространенный случай - локальный URL. Поэтому иногда url надо дополнять до полного
			if( !preg_match('#^http://#',$a->href) )$a->href = 'http://trikotyata.ru'.$a->href;
