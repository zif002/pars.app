...
function getBigImage($url,$i=1){
	...
	if( count($data->find('.itemMainLink')) ){
		$img =  $data->find('.itemMainLink',0);

			 	
		// }
		//echo $img;
		if( !preg_match('#^http://#',$img->href) )$img->href = 'http://vashislasti-opt.ru'.$img->href;	
	  	//echo $img->href;
	  	$ch = curl_init();  
	...
	if($flag){
		...
		if(count($data->find('.price'))){	 		
		 ...
		 if(count($data->find('.brand'))){	 		
		  	$article = $data->find('.brand',0);		  
		 ...
		 //параметры
		 if(count($data->find('.desc'))){	
			$params_temp = $data->find('.desc',0);  	
...
function getYandexImages($url,$findpages = true,$i=1,$n=200){
	...
	if( $findpages and count($data->find('p.name a'))){
		foreach($data->find('p.name a') as $a){	
			//$a->href.'<br>';
			// довольно распространенный случай - локальный URL. Поэтому иногда url надо дополнять до полного
			if( !preg_match('#^http://#',$a->href) )$a->href = 'http://vashislasti-opt.ru'.$a->href;
