...
function getBigImage($url,$i){
	...
	if($r_photo){
		...
		// }
		 ...
		 // }
		 //Вывод цены
		 if(count($data->find('.big'))){
		 		//echo "<span>Цена в магазине </span>";
		 	$price = $data->find('.big',0);
			$price = "Цена: ".$price->plaintext;
		 	echo  $price."<br>";
		
		 }else{			
		
		 }
		 if(  count($data->find('.panel-body .col-xs-8'))){
		 	//echo "<span style='text-transform:uppercase;'>Ваша цена </span>";
		 	$properties = $data->find('.panel-body .col-xs-8',0);
		 	$properties =$properties->plaintext;
		 	echo $properties->plaintext."<br>";
		 ...
		 if(  count($data->find('.col-xs-16 p'))){
		 	...
		 	$descripition = $descripition->plaintext;
		 ...
		 // 	}
		 // 	$descript_str =  implode(" ",$descript_arr);
		 // }else{			
