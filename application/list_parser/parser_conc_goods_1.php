...
function getBigImage($url,$i=1){
	...
	if( count($data->find('#image_wrap')) ){
		$img =  $data->find('#image_wrap img',0);
		$img = $img->src;
			 	
		// }
		echo $img;
		if( !preg_match('#^http://#',$img) )$img = 'http://www.stationerymarket.ru'.$img;	
	  	//echo $img->href;
	  	$ch = curl_init();  
	...
	if($flag){
		...
		//Вывод названия товара
		 if( count($data->find('h1.name')) ){
		 	$product_title = $data->find('.h1.name',0);	
		...
		//Вывод цены
		if(count($data->find('.article_art_good'))){	 		
		 	$article = $data->find('.article_art_good',0);		 
		...
		//Вывод цены
		if(count($data->find('.article_pr'))){	 		
		 	$price = $data->find('.article_pr',0);		 
...
function getYandexImages($url,$findpages = true,$i=1,$n=3){
	...
	if( $findpages and count($data->find('.article_name '))){
		foreach($data->find('.article_name ') as $a){	
			//$a->href.'<br>';
			// довольно распространенный случай - локальный URL. Поэтому иногда url надо дополнять до полного
			if( !preg_match('#^http://#',$a->href) )$a->href = 'http://www.stationerymarket.ru'.$a->href;
