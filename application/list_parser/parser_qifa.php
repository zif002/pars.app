...
function getBigImage($url,$i=1){
	...
	if( count($data->find('#img_0')) ){
		$img =  $data->find('#img_0',0);
		//echo $img->href;
			 	
		// }
		//echo $img;
		if( !preg_match('#^http://#',$img->href) )$img->href = 'http://www.qifa.ru'.$img->href;	
	  	//echo $img->href;
	  	$ch = curl_init();  
	...
	if($flag){
		...
		//Вывод названия товара
		 if( count($data->find('.card-product_name')) ){
		 	$product_title = $data->find('.card-product_name',0);	
		...
		// Артикул
		//Вывод цены
		if(  count($data->find('.b-product__sku'))){	 		
		 	$article = $data->find('.b-product__sku',0);	 
		...
		//Вывод цены
		if(  count($data->find('.card-product_summ_new'))){	 		
		 	$price = $data->find('.card-product_summ_new',1);		 
		 ...
		 if(count($data->find('.card-product_descr'))){
		 	$size1 = array();
			foreach($data->find('.card-product_descr p') as $size) {	  	
					$size = $size->plaintext;
					$size1[] = $size;
					echo $size."<br>";
...
function getYandexImages($url,$findpages = true,$i=1,$n=3){
	...
	if( $findpages and count($data->find('.search-results_sheet_name a'))){
		foreach($data->find('.search-results_sheet_name a') as $a){	
			//$a->href.'<br>';
			// довольно распространенный случай - локальный URL. Поэтому иногда url надо дополнять до полного
			if( !preg_match('#^http://#',$a->href) )$a->href = 'http://www.qifa.ru'.$a->href;
